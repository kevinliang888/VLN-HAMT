...
class Seq2SeqCMTAgent(BaseAgent):
    ...
    def find_all_follow_vps(self, s, ch):
        return [s[i + 1][0] for i, ltr in enumerate(s) if ltr == ch and (i + 1) < len(s)]
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True):
        ...
        with torch.autograd.set_detect_anomaly(True):
            ...
            for t in range(self.args.max_action_len):
                ...
                # add
                # if self.args.clean_hist:
                #     for ob_id, ob in enumerate(obs):
                #         path = traj[ob_id]['path']
                #         last_vp = path[-1]
                #         prev_path = path[:-1]
                #         if last_vp in prev_path:
                #             # Delete history embeddings for batch size 1
                #             ind_last_vp = prev_path.index(last_vp)
                #             if ind_last_vp == 0:
                #                 hist_embeds = [self.vln_bert('history').expand(batch_size, -1)]
                #                 hist_lens = [1 for _ in range(batch_size)]
                #             else:
                #                 hist_embeds = hist_embeds[:ind_last_vp]
                #                 hist_lens[ob_id] = len(hist_embeds)
                ...
                #     if last_vp in prev_path:
                #         follow_vps = self.find_all_follow_vps(prev_path, last_vp)
                #         for c_id, c in enumerate(ob['candidate']):
                #             if c['viewpointId'] in follow_vps:
